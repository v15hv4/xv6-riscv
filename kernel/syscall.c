...
static uint64 (*syscalls[])(void) = {
    ...
    [SYS_waitx] sys_waitx, [SYS_trace] sys_trace};

char *syscall_name[] = {
    "",      "fork",  "exit",   "wait",   "pipe",  "read",  "kill",   "exec",
    "fstat", "chdir", "dup",    "getpid", "sbrk",  "sleep", "uptime", "open",
    "write", "mknod", "unlink", "link",   "mkdir", "close", "waitx",  "trace",
};
...
void syscall(void) {
  ...
  if (num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    int arg0 = p->trapframe->a0, argcount = 0;

    if (num == SYS_wait || num == SYS_pipe || num == SYS_kill ||
        num == SYS_chdir || num == SYS_dup || num == SYS_sbrk ||
        num == SYS_sleep || num == SYS_unlink || num == SYS_mkdir ||
        num == SYS_close || num == SYS_trace)
      argcount = 1;
    else if (num == SYS_exec || num == SYS_fstat || num == SYS_open ||
             num == SYS_link) {
      argcount = 2;
